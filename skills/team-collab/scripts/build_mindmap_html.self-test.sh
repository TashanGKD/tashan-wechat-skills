...
grep -q "className='taxline'" "$H" && grep -q "className='taxtick'" "$H" && pass "D 左侧纵向时间坐标轴+刻度" || bad "时间坐标轴缺失"
# 剩余 P1
grep -q 'tree.slim.json' "$H" && grep -q 'tree.slim.json' "$TMP/tree/serve.py" && pass "#7 瘦索引(serve预生成+画布拉slim)" || bad "#7 瘦索引缺失"
grep -q 'name="viewport"' "$H" && grep -q "addEventListener('touchstart'" "$H" && grep -q 'touch-action:none' "$H" && pass "#12 移动端(viewport+触摸+touch-action)" || bad "#12 移动端缺失"
grep -q 'await new Promise(r=>setTimeout(r))' "$H" && pass "#6 大对话解析分片让出主线程(不冻UI)" || bad "#6 分片解析缺失"
