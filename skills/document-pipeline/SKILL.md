...
> **与旧 Skill 的关系**：wechat-article-writer 为本 Skill 的 thin wrapper（公众号入口），触发词不变，内部转发到此处
>
> **⚙️ 适配说明**：本仓库聚焦公众号发文链路，实际使用 `target_format = html`（公众号）与 `md`（普通草稿）。research 模式（Stage 8D 认知结构/知识图谱注册）与「内部经验→开发规范」路由为原始工作区遗留分支，默认不启用——相关 Stage 即便保留也不会被公众号任务触发。文章输出到你工作空间的「公众号文章」目录。
...
| 层级 | 文档 | 需要理解的概念 |
|---|---|---|
| **D0 写作根（必读）** | `references/写作习惯与风格手册.md` | 写作风格偏好：文章结构/语言风格/禁用套话/标题原则 |
| **D3 规范参考** | `../article-proofreading/SKILL.md`（作为审稿标准参考，非触发执行）| 五轮审稿标准：AI腔/标题错误/绝对表达/结构层次/结语完整性 |
...
④ 引用规范在审稿修改后、格式转换前强制执行：正文引用与文末参考文献必须一一对应
...
A. 判断 target_format：
   ...
   **文档性质判断（写入位置决策）**：
   ...
   - **内部经验积累类**（实际执行中产生的踩坑记录/最佳实践/操作规范）→ 写入总规范库
     ...
     路由规则：确认主题分类 → 写入 `workspace/开发规范/[主题]/踩坑速查.md` 或 `最佳实践.md`
...
B. 判断入场 Stage（规则1优先于规则2，触发词优先）：
   ...
   - 用户提供了已审稿的 MD + 说「转 HTML/转 LaTeX/发布」→ 入场 Stage 7.3 引用规范检查，通过后进入 Stage 8
...
Stage 0 输出：[target_format, entry_stage, dimension（research 模式）]
  ...
  ⚠️ 单任务声明：整条流水线（Stage 0-8）视为一次完整任务，中间 Stage 不单独收尾。
```
...
Step 1.1.5  认知文档引用扫描（文章主题来自用户认知结构时执行）
  当文章需要引用用户认知结构中的内容时：
  a. 先在 workspace/认知结构/L1_系统性文档/ 目录下做全局 Glob 扫描
...
Step 1.3  写草稿（遵循 Step 1.1 风格约束）
  ...
  写作要求：
    - 重要主张标注来源 [来源N] 或 [待核实]
    - 关键概念首次出现时定义
    - 摘要/开头/章节第一句优先使用正面定义，禁止用 `X 不是 Y，而是 Z` 制造不必要反差；能改成 `X 是 Z` 的必须改
    - 段落间有显式的逻辑连接（不允许段落跳跃）
    - 结尾必须有「## 参考资料」节（格式：[来源N] 作者 | 标题 | URL | 日期）

Step 1.4  结构完整性自检（写完草稿后立即执行，进入 Stage 2 前）
  
  检查：文章中是否包含「N 项列表」类结构？
  （判断标准：摘要/概览/章节中明确写了「X 种模式」「N 个步骤」「以下 K 类」等枚举性表述）

  IF 存在 N 项列表：
    对照枚举项，逐项确认「正文是否有对应的独立展开段落/小节」
    生成检查清单：
    「📋 结构完整性检查：
      □ 项目1：[标题] — [✅ 有展开 / ❌ 缺失展开]
      □ 项目2：[标题] — [✅ / ❌]
    ...
    若有 ❌ 缺失项：
    → 【必须立即补写】在进入 Stage 2 前，先补全缺失的展开内容
    → 补写后重新标注该项为 ✅
    → 全部 ✅ 后才输出最终草稿

  IF 不存在 N 项列表 → 跳过，直接输出草稿

  ⚠️ 禁止：在结构不完整（有 ❌ 条目）的情况下继续 Stage 2
...
**DMXAPI 调用参数**：
```
API Key: 读取环境变量 $DMXAPI_KEY（⛔ 禁止在 skill/仓库中硬编码密钥）
...
Step 5.1  执行图片生成（调用 `ai-image-generator` Skill）
    ...
    加载：`../ai-image-generator/SKILL.md`
...
Step 7.3  引用规范检查（进入 Stage 8 之前必做）
  
  ⚠️ 此步骤在「审稿修改完成 → 进 Stage 8」之间强制执行，不可跳过。
  
  适用范围：
    - 含外部文献、网页、机构数据、论文事实、具体数字/百分比/年份事件的文章
    - target_format = html 的公众号文章
  
  检查与修复：
    1. 逐段扫描事实性主张，确认正文中有对应引用编号；缺失来源 → 标注「[待补引用：说明缺什么来源]」，不得直接进入 Stage 8。
    2. 检查文末参考文献列表：正文出现的编号必须存在，未被正文引用的文献应删除或说明保留原因。
    3. 公众号/HTML 输出统一使用正文上标编号：`<sup style="font-size: 12px; line-height: 0; vertical-align: super; color: rgb(111,167,170);">[n]</sup>`。
    4. 禁止把正文写成「（见文献 [n]）」或「见文献 [n]」这类口语化引用；转换时改为紧贴句末的上标编号。
    5. 多篇并列引用写作 `[1,3]`，连续引用写作 `[1-3]`；编号必须与文末列表一致。
    6. 图片图注只保留面向读者的来源说明；删除「访问日期」「后续公开发布」「核对图片许可」「提示词」「官方图示拼接」等内部核对残留。
  
  完成时输出：
    「引用规范：正文上标/引用标注 [N] 处；文末参考文献 [M] 条；待补引用 [K] 处。」
    若 K > 0 → 停下，等待用户补来源或确认删除对应事实性主张。

Step 7.4  配图门控检查（进入 Stage 8 之前必做）
  
  ⚠️ 此步骤在「引用规范检查完成 → 进 Stage 8」之间强制执行，不可跳过。
  ...
  IF target_format = html AND Stage 4-6（画图规划/批量画图/图片自审）尚未执行：
    ...
    未发现适合配图的位置（纯文字/表格型文章）→ 直接进入 Step 7.5
  
  ELSE（Stage 4-6 已执行，或 target_format != html）→ 直接进入 Step 7.5
...
分叉 A：target_format = html
  ...
  Step A.2  转换为微信 HTML
    参照 references/微信公众号发布手册.md §2 HTML 规范
    图片：base64 嵌入，不引用本地路径
    引用：按发布手册 §2.8 转为正文上标编号，文末参考文献编号一致
    颜色：H2 / 关键H3 / 底部署名标签 / 协会简介 / 二维码提示文字统一用 rgb(111,167,170)
    strong：#1a1a1a（加粗但不染色）
  
  Step A.3  添加底部模板
    参照发布手册 §4.3：署名 → 分隔线 → TA SHAN logo → 协会简介 → 二维码 → 官网说明
    官网说明必须使用当前默认文案，并包含 world.tashan.chat
    即使是“轻量活动推送”（正文只有导语 + 报名引导句 + 活动图），也不得省略固定底部模板
  
  Step A.4  排版核查
    - 若团队 Windows 侧存在 C:/Temp/diff_check.py → 先运行，必须 0 处不一致
    - 若团队 Windows 侧存在 C:/Temp/check_html.py → 再运行，必须 15项全部 [OK]
    - 当前工作区未收录上述脚本时 → 按发布手册 §5 手动完成内容对照 + 15项核查 + 引用规范核查
    - 活动转发类若采用“轻量活动推送”结构，额外核查：导语顺序是否为“背景 → 时间/地点 → 发起/协办/支持”，以及文末是否保留完整固定结尾
...
分叉 C：target_format = md + mode != research
  **文档性质检查（Stage 0 已判断，此处执行保存路由）**：
  ...
  - 若 Stage 0 判断为**原创思考类**（用户自己的分析/产品定义/认知文档）：
    ...
    → 认知结构类：保存到 `workspace/认知结构/L1_系统性文档/[维度]/[文件名]_YYYYMMDD.md`
  ...
  - 若 Stage 0 判断为**内部经验积累类**（踩坑/最佳实践/操作规范）：
    ...
    → 路径：`workspace/开发规范/[主题]/[类型].md`
    → 主题子目录不存在时自动创建
    → 写入后在 `workspace/开发规范/_开发规范总索引.md` 中追加一行索引（若索引文件存在）
...
分叉 D：target_format = md + mode = research
  Step 8.1  确认保存路径
    路径：workspace/认知结构/L1_系统性文档/[dimension]/知识库/[文件名]_YYYYMMDD.md
    目录不存在 → 自动创建
  ...
  Step 8.4  三处注册（research 模式专属，不可跳过）
    ...
    ② 更新 知识图谱_正式文档.md（REF-EXT 区段）
       路径：workspace/认知结构/知识图谱_正式文档.md
    ...
    ③ 追加系统日志：workspace/认知结构/L3_原始记录/系统日志.md
...
| 用户说「只转格式，其他都好了」| 若含外部事实/参考文献，先 Stage 7.3；否则直接 Stage 8 |
...
### v1.8 → v1.9 — 2026-03-25 — Stage 1 新增「结构完整性自检」（Step 1.4）

**根因**：project-retrospective-20260325 B类发现。写公众号推文「把大脑分层外化」时，Stage 1 正文列出了 5 种初始构建模式，但 AI 只展开了模式1/2/4，跳过了模式3和5，需用户追问才补完。根本原因：Stage 1.3 无「N项列表完整性」检查机制，AI 写完就进入下一步，无自检。

**经验核心**：正文中凡有「N 种模式/K 类方法/X 步骤」等枚举性表述，必须在草稿完成后对照逐项确认每项都有独立展开；有缺失立即补写，不允许带 ❌ 进入 Stage 2。

**修改内容**：
- 新增：Stage 1 增加 Step 1.4「结构完整性自检」——写完草稿后检查所有 N 项列表是否全部展开，有缺失立即补写，全部 ✅ 后才输出草稿

**验证结果**：
- 正向验证：下次写包含「N种/K类」结构的文章时，Stage 1 结束后应出现 Step 1.4 完整性检查清单
- 负向验证：无枚举性列表的文章跳过 Step 1.4，不影响流程

**备份路径**：`history/SKILL_v1.8_20260325_before_completeness.md`

**验证状态**：🔵 待验证

---

### v1.9 → v1.10 — 2026-05-22 — Stage 7 新增引用规范检查

**根因**：刘子笑 AI4S 公众号推送版真实修订中发现，正文使用「见文献 [n]」而不是规范上标，图注与参考文献还混入「访问日期」「后续公开发布建议核对图片许可」「官方图示拼接」等内部核对残留。若没有独立检查点，审稿通过后直接转 HTML 会把内部工作痕迹带入公开稿。

**修改内容**：
- 新增：Stage 7.3「引用规范检查」——审稿修改后、Stage 8 格式转换前强制执行
- 修改：原 Stage 7.3 配图门控顺延为 Stage 7.4，完成信号顺延为 Stage 7.5
- 修改：已审稿 MD 直接转格式时，先入场 Stage 7.3，避免绕过引用规范
- 新增：Stage 8A 转 HTML 时按发布手册 §2.8 使用正文上标编号，文末参考文献编号一致
- 新增：强绑定规则 R9 CITATION_GATE

**验证方法**：
- 正向验证：下次公众号文章含参考文献时，Stage 7 后应输出「引用规范：正文上标/引用标注 N 处；文末参考文献 M 条；待补引用 K 处」
- 负向验证：若正文存在「见文献 [n]」或图注残留「访问日期/图片许可/提示词」，不得进入 Stage 8

**验证状态**：🟡 已用刘子笑 AI4S 推送版案例回填验证，后续新稿待验证
