...
| 优先级 | 模型 | 适用场景 | API Key |
|---|---|---|---|
| **首选** | `qwen-image-2.0-pro` | 含中文文字的信息图、结构图 | 项目运行时优先取 `AI_GENERATION_API_KEY`；直连 DashScope 原生图片接口时兼容 `$DASHSCOPE_API_KEY` |
| **次选** | `wan2.6-t2i` | 纯视觉风格图（无需复杂中文文字排版）| 同上（DashScope）|
| 备用 | `gpt-image-1`（DMXAPI）| 英文为主的极简图 | 环境变量 `$DMXAPI_KEY`（见下方"API 密钥配置"）|

### API 密钥配置

> ⚠️ **安全原则**：API 密钥不得硬编码在 skill / 仓库中，一律从环境变量读取。
>
> 直连 DashScope 原生图片接口时，按顺序取 key：
> 1. 环境变量 `DASHSCOPE_API_KEY`（首选，直连 DashScope 图片端点）
> 2. 环境变量 `AI_GENERATION_API_KEY`
> 3. 环境变量 `LLM_API_KEY`（最后兜底）
>
> ⚠️ 注意：形如 `sk-sp-...` 的后端代理 key **不能直连 DashScope 图片接口**（返回 401）。
>
> 🔑 未设置 key 时配图步骤无法生成图片；请设置 `DASHSCOPE_API_KEY`（或 `AI_GENERATION_API_KEY`）后再执行配图。⛔ 切勿把密钥写进 skill 或提交到仓库。
>
> ⛔ **禁止**：遇到 401 后放弃 qwen-image、改用内置 GenerateImage 工具——所有信息图必须用 qwen-image-2.0-pro 生成。
>
> `DMXAPI_KEY` 仅保留给 `gpt-image-1` 备用链路。
...
API_KEY = (
    os.environ.get('AI_GENERATION_API_KEY')
    or os.environ.get('DASHSCOPE_API_KEY')
    or os.environ.get('LLM_API_KEY', '')
)
if not API_KEY:
    raise ValueError("请先配置 AI_GENERATION_API_KEY（或 DASHSCOPE_API_KEY / LLM_API_KEY）")
...
风格库文件：`references/配图风格库.md`（当前真源；`图片风格库.md` 为兼容镜像）
...
**参考图库**：（本组织空间暂未收录参考图库；如需可在本 skill 的 `references/` 下自建 `图片参考库/`）
...
如果文章还没有配图索引，立即创建（模板：`../wechat-article-writer/templates/配图索引模板.md`）。
...
### v1.2 — 2026-03-31 — 补充可用 key 查找路径，禁止遇 401 后降级内置工具

**根因**：2026-03-31 写作 SkillHub 推文时，AI 从 `tashan-world/.env` 取到 `sk-sp-...`（代理 key），直连 DashScope 返回 401，放弃后改用 Cursor 内置 GenerateImage 工具，违反了「所有信息图必须用 qwen-image-2.0-pro」的规范。根本原因：Skill 的 key 配置节未说明代理 key 不能直连图片端点，也未指向工作区中已有的可用脚本 `gen_qwen_images.py`。

**经验核心**：根因类型 = 规范质量问题（key 路径列表不完整，缺少可直连的真实来源）

**修改内容**：
- 修改：API 密钥配置节 → 新增注意事项「`sk-sp-...` 是代理 key，不能直连 DashScope」
- 新增：查找顺序，第1优先级指向 `workspace/产品定义/gen_qwen_images.py`（已验证可用）
- 新增：⛔ 明确禁止「遇 401 后放弃 qwen-image，改用内置工具」

**验证结果**：
- 正向验证：下次写文章需要配图时，遇到 401 应先查 gen_qwen_images.py 找到正确 key
- 负向验证：纯文字任务不触发本 Skill

**验证状态**：🔵 待验证

**备份路径**：`history/SKILL_v1.1_20260331.md`
...
- 新增：参考图库路径 `workspace/产品定义/图片参考库/架构图-全景对象关系型/`
