...
>
> ⚠️ **macOS / Linux 注意**：系统自带的 `python3` 往往**没装** chromadb/sentence-transformers（且通常没有 `python`/`py`
> 命令），脚本探测不到可用解释器就会停。最省心是**建一个独立 venv 并用它**（几 GB 依赖放仓库外、跨项目复用）：
> ```bash
> python3 -m venv ~/.venvs/tc-vector
> ~/.venvs/tc-vector/bin/pip install chromadb sentence-transformers
> export TC_VECTOR_PYTHON=~/.venvs/tc-vector/bin/python   # 之后照抄 python3 …build_memory_index.py 会自动转到它
> ```
...
解释器（比如某个 conda 环境的 python）；③ **建独立 venv（推荐，尤其 macOS/Linux，见上「macOS / Linux 注意」）**：
`python3 -m venv ~/.venvs/tc-vector && ~/.venvs/tc-vector/bin/pip install chromadb sentence-transformers`，
再 `export TC_VECTOR_PYTHON=~/.venvs/tc-vector/bin/python`。
