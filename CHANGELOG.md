...
### Added
- 新增 `team-collab` skill：通用团队异步协作规范 + 记忆层（「文件即消息」的 `团队协作记录/` 频道、发帖/回帖/决策留痕、找谁负责、工作日志、会话归档与「对话树」）。**仓库无关**——可落地到任意 git 项目（含新仓库 bootstrap 说明）。自带 `scripts/`（对话树/归档/校验，多数带 self-test）。
- 该 skill 由 AI_PaperWriter 项目通用化而来，去除项目专属绑定。

### Changed
- 仓库定位从「公众号 skill 集」扩展为「他山团队通用 skill 集」（写作 + 抓取 + 协作），skill 数 8 → 9。
...
### Added
- 新增 `wechat-article-crawler` skill：抓取 / 归档微信公众号文章。汇总 6 种抓取方案（官方 API / 手动链接 / mitmproxy / werss RSS / 搜狗 / 无头浏览器）与「环境异常」绕过策略，自带单篇抓取脚本 `scripts/fetch_wechat.py`。
- 随该 skill 收录他山学科交叉公众号 14 篇**自有**文章存档（`references/他山文章存档/` + `存档索引.md`），作为写作样本 / 风格对照。第三方媒体全文因版权不入仓库。

### Changed
- 仓库范围从「发文链路」扩展为「公众号 skill 集：发文 + 抓取归档」（skill 数 7 → 8）。
