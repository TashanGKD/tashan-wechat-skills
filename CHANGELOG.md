...
### Changed
- 明确**核心用法**：把已有手稿（word/md/txt）排版并导出为微信 HTML（含加内容摘要、完善引用规范、套底部模板）； 先抽取为 markdown 再进流程。
- **审稿改为可选**：、（Stage 7 + R8）、 及发布手册核查清单中，审稿由「强制门控」调整为「可选、按需触发」；排版导出类任务默认跳过审稿。
- 入口  执行模式新增「排版导出模式」为首要路径；触发词补「排版 / 转 HTML / 导出 HTML」。
